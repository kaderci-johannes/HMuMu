 ...
 */
// std::string __inputfilename = "/afs/cern.ch/user/m/malhusse/h2mu/synch_files.txt";
// std::string __outputfilename = "ntuple.root";
...
std::vector<int> vVBFTight, vggFTight, vVBFLoose, v01JetsTight, v01JetsLoose,
	vTotal;
...
bool passBTaggedJetVeto(Jets *jets)
{
	for (Jets::const_iterator it = jets->begin(); it != jets->end(); ++it)
		if (it->_pt > 20. & TMath::Abs(it->_eta) < 2.4 && it->_btag[0] > 0.4941)
			return false;
	return true;
}
				...
				MET const &met, Event const &event)
{
	...
	for (Jets::const_iterator it = jets->begin(); it != jets->end(); ++it)
	{
		if (it->_pt > 20 && TMath::Abs(it->_eta) < 4.7)
		{
	...
	if (isPreSelected)
	{
		...
		{
			__nVBFLoose++;
			vVBFLoose.push_back(event._event);
			return;
		}
	}
	if (!isPreSelected)
...
void process()
{
	...
	for (uint32_t i = 0; i < numEntries && __continueRunning; i++)
	{
		...
		if (muonPairs.size() == 0 || muonPairs.size() > 1)
			continue;

		__nTotalPassMuonSelections++;
		if (passElectronVeto(electrons, muonPairs[0].first, muonPairs[0].second) && passBTaggedJetVeto(jets))
		{
	...
	ofstream fVBFTight, fggFT, fvbFL, f01JetsTight, f01JetsLoose, fTotal;
	...
	std::cout << "Yields: " << std::endl
			  ...
			  << "__nVBFLoose = " << __nVBFLoose << std::endl
			  ...
			  << "--------------------------------" << std::endl
			  << "TOTAL = " << __nVBFTight + __nggFTight + __nVBFLoose + __n01JetsLoose + __n01JetsTight << std::endl;
...
int main(int argc, char **argv)
{
	...
	desc.add_options()("help", "produce help messages")("input", po::value<vector<string>>(), "a file specifying all the ROOT files to process")("isMC", po::value<bool>(), "type of data: DATA vs MC")("output", po::value<vector<string>>(), "output file name");
